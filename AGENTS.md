...
5. **ParticleBatchOptions** (see `src/emitter/types.ts` and README): `lifeTime`, `count`, `size`, `origin`, `v0`, `velocityBias`, `omega0`, `gravity`, `spawnDuration`, `Cd`, `Cr`, `density`, `area`, `mass`, `momentOfInertia`, `atlas` (offset + optional sweep), `attack` / `decay` (`ParticleEnvelope`: mode `none`|`size`|`opacity`, `duration` as fraction of lifetime, cubic-bezier; attack from life start, decay toward life end; factors multiply), `spawnSize`. `processParticleBatchOptions()` fills defaults and derives `drag` and `angularDrag`.
