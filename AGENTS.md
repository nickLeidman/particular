...
5. **ParticleBatchOptions** (see `src/emitter/types.ts` and README): `lifeTime`, `count`, `size`, `origin`, `v0`, `velocityBias`, `omega0`, `gravity`, `spawnDuration`, `Cd`, `Cr`, `density`, `area`, `mass`, `momentOfInertia`, `atlas` (offset + optional sweep), `decay` (mode `none`|`size`|`opacity`, timing fractions, cubic-bezier), `spawnSize`. `processParticleBatchOptions()` fills defaults and derives `drag` and `angularDrag`.
